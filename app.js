...
function addParapet(width, depth, y, z, parent = roofs, frontGap = null, sideGap = null) {
  const h = 0.36;
  const addSideSegments = (side, gap) => {
    const sideMin = z - depth / 2 - 0.35;
    const sideMax = z + depth / 2 + 0.35;
    const gapMin = gap.z - gap.depth / 2;
    const gapMax = gap.z + gap.depth / 2;
    const frontPart = Math.max(0, gapMin - sideMin);
    const rearPart = Math.max(0, sideMax - gapMax);
    const sideX = side === "left" ? -width / 2 - 0.18 : width / 2 + 0.18;
    const capX = side === "left" ? -width / 2 - 0.28 : width / 2 + 0.28;
    if (frontPart > 0.05) {
      trimBox({ x: 0.34, y: h, z: frontPart }, { x: sideX, y, z: sideMin + frontPart / 2 }, parent);
      trimBox({ x: 0.22, y: 0.12, z: frontPart }, { x: capX, y: y + 0.27, z: sideMin + frontPart / 2 }, parent);
    }
    if (rearPart > 0.05) {
      trimBox({ x: 0.34, y: h, z: rearPart }, { x: sideX, y, z: gapMax + rearPart / 2 }, parent);
      trimBox({ x: 0.22, y: 0.12, z: rearPart }, { x: capX, y: y + 0.27, z: gapMax + rearPart / 2 }, parent);
    }
  };
  if (frontGap) {
  ...
  trimBox({ x: width + 0.7, y: h, z: 0.34 }, { x: 0, y, z: z + depth / 2 + 0.18 }, parent);
  if (sideGap && sideGap.side === "left") {
    addSideSegments("left", sideGap);
  } else {
  ...
  if (sideGap && sideGap.side === "right") {
    addSideSegments("right", sideGap);
  } else {
    trimBox({ x: 0.34, y: h, z: depth + 0.7 }, { x: width / 2 + 0.18, y, z }, parent);
    trimBox({ x: 0.22, y: 0.12, z: depth + 0.78 }, { x: width / 2 + 0.28, y: y + 0.27, z }, parent);
  }
...
function addMiddleRoofTerrace() {
  ...
  box("fourth floor cutout glass guard open side", { x: 0.06, y: railH, z: d - 0.44 }, { x: leftEdge, y: y + railH / 2, z: z + 0.04 }, materials.glass, balconies, false);

  trimBox({ x: w - 0.3, y: 0.1, z: 0.12 }, { x, y: y + railH + 0.06, z: frontEdge - 0.04 }, balconies);
  trimBox({ x: 0.12, y: 0.1, z: d - 0.32 }, { x: leftEdge - 0.02, y: y + railH + 0.06, z: z + 0.04 }, balconies);
...
function addTerraceInnerParapet() {
  ...
  box("terrace inner parapet return at notch", { x: 0.24, y: h, z: innerReturnDepth }, { x: rightEdge, y, z: innerReturnZ }, materials.wall, roofs);
  trimBox({ x: 0.12, y: 0.1, z: innerReturnDepth + 0.12 }, { x: rightEdge + 0.04, y: y + 0.23, z: innerReturnZ }, roofs);
}
...
function buildHouse() {
  ...
  addParapet(7.5, 10.4, 12.85, 0, roofs, { x: -1.6, width: 3.6 }, { side: "left", z: -3.525, depth: 3.35 });
  ...
  addWindow(-0.005, 11.2, -3.525, 0.82, 1.62, "left");
